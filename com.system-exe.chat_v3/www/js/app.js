...
angular.module('starter').controller('mainCtrl', ['$rootScope', '$scope', '$ionicPlatform', '$ionicModal', '$ionicLoading', '$timeout', '$ionicScrollDelegate', '$systemAuth', '$systemCommon', '$systemConfig', '$ionicActionSheet', '$ionicSideMenuDelegate', '$firebaseArray', '$systemSql', function($rootScope, $scope, $ionicPlatform, $ionicModal, $ionicLoading, $timeout, $ionicScrollDelegate, $systemAuth, $systemCommon, $systemConfig, $ionicActionSheet, $ionicSideMenuDelegate, $firebaseArray, $systemSql) {
    ...
    $scope.initModel = function() {
        $scope.model = {
            ...
            allowGetUser: true,
            db: null,
            arrayMessageColumn: ['body', 'pair', 'fromUser', 'toUser', 'time', 'timeago']
        };
    };
    $scope.init = function() {
        //check authen
    ...
    $scope.checkAndInsertMessage = function() {
        $systemSql.openDb().then(function(success) {
            console.log('db', success);
            $scope.model.db = success;
            $systemSql.executeQuery(success, 'select * from messages_' + $scope.model.userAuth.expires + ' order by time asc').then(function(success) {
                console.log('select', success);
                if (success.rows.length > 0) {
                    var newArray = [];
                    for (var i = 0; i < success.rows.length; i++) {
                        newArray.push(success.rows[i]);
                    }
                    // console.log(content);
                    var newMessageListSortByTime = newArray.sort(function(item1, item2) {
                        return item1.time < item2.time;
                    });
                    console.log('newMessageListSortByTime', newMessageListSortByTime);
                    var date = new Date();
                    var finalDay = newMessageListSortByTime[0].time.substr(0, 10);
                    var newDate7 = new Date(date.setDate(date.getDate() - 7)).toISOString().substr(0, 10);
                    var nowDate = new Date().toISOString().substr(0, 10);
                    if (finalDay == nowDate) {
                        return;
                    }
                    if (newDate7 < finalDay && finalDay < nowDate) {
                        $systemCommon.getMessages('from', $scope.model.userAuth.password.email).then(function(success) {
                            console.log('message', success);
                            $scope.saveMessageFromfinalDayToLessThanToDay(success, finalDay);
                        }, function(err) {});
                        $systemCommon.getMessages('to', $scope.model.userAuth.password.email).then(function(success) {
                            console.log('message', success);
                            $scope.saveMessageFromfinalDayToLessThanToDay(success, finalDay);
                        }, function(err) {});
                    } else {
                        $systemCommon.getMessages('from', $scope.model.userAuth.password.email).then(function(success) {
                            console.log('message', success);
                            $scope.saveMessage7Day(success, finalDay);
                        }, function(err) {});
                        $systemCommon.getMessages('to', $scope.model.userAuth.password.email).then(function(success) {
                            console.log('message', success);
                            $scope.saveMessage7Day(success, finalDay);
    ...
    $scope.saveMessageFromfinalDayToLessThanToDay = function(successList, date) {
        var count = (new Date().getTime() - new Date(date)) / (1000 * 24 * 3600);
        for (var i = 1; i < count; i++) {
            eval('var newDate' + i + ' = new Date(date.setDate(date.getDate() - ' + i + ')).toISOString().substr(0, 10);')
        }
        var newList = successList.filter(function(item) {
            return ((angular.isDefined(newDate1) && item.time.indexOf(newDate1)) || (angular.isDefined(newDate1) && item.time.indexOf(newDate2)) || (angular.isDefined(newDate1) && item.time.indexOf(newDate3)) || (angular.isDefined(newDate1) && item.time.indexOf(newDate4)) || (angular.isDefined(newDate1) && item.time.indexOf(newDate5)) || (angular.isDefined(newDate1) && item.time.indexOf(newDate6)) || (angular.isDefined(newDate1) && item.time.indexOf(newDate7)));
        });
        if (newList != null && newList.length > 0) {
            newList.forEach(function(item) {
                $scope.insertMessageToDatabase(item);
            });
        }
    };
    $scope.saveMessage7Day = function(successList) {
        var date = new Date();
        var newDate1 = new Date(date.setDate(date.getDate() - 1)).toISOString().substr(0, 10);
        var newDate2 = new Date(date.setDate(date.getDate() - 2)).toISOString().substr(0, 10);
        var newDate3 = new Date(date.setDate(date.getDate() - 3)).toISOString().substr(0, 10);
        var newDate4 = new Date(date.setDate(date.getDate() - 4)).toISOString().substr(0, 10);
        var newDate5 = new Date(date.setDate(date.getDate() - 5)).toISOString().substr(0, 10);
        var newDate6 = new Date(date.setDate(date.getDate() - 6)).toISOString().substr(0, 10);
        var newDate7 = new Date(date.setDate(date.getDate() - 7)).toISOString().substr(0, 10);
        var newList = successList.filter(function(item) {
            return (item.time.indexOf(newDate1) || item.time.indexOf(newDate2) || item.time.indexOf(newDate3) || item.time.indexOf(newDate4) || item.time.indexOf(newDate5) || item.time.indexOf(newDate6) || item.time.indexOf(newDate7));
        });
        if (newList != null && newList.length > 0) {
            newList.forEach(function(item) {
                $scope.insertMessageToDatabase(item);
    ...
    $scope.insertMessageToDatabase = function(item) {
        var arrayMessageValue = [item.body, item.pair, item.from, item.to, item.time, item.timeago];
        $systemSql.insertData($scope.model.db, 'messages_' + $scope.model.userAuth.expires, $scope.model.arrayMessageColumn, arrayMessageValue);
    };
    ...
    $scope.createNewUserSubmit = function() {
        ...
        $systemAuth.sigup($scope.model.newUser).then(function(success) {
            ...
            $systemAuth.login($scope.model.newUser.email, $scope.model.newUser.password).then(function(success) {
                ...
                $systemSql.openDb().then(function(success) {
                    $scope.model.db = success;
                    $systemSql.createTable($scope.model.db, 'messages_' + $scope.model.userAuth.expires, $scope.model.arrayMessageColumn).then(function(success) {
                        console.log(success);
                        //hide loading
                        $scope.loading.hide();
                    }, function(err) {
